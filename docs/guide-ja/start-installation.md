...
[PHP の内蔵ウェブ・サーバ](https://php.net/manual/ja/features.commandline.webserver.php) を使ってください。
...
また、アプリケーションがデータベースを必要とする場合は、[PDO PHP 拡張](https://php.net/manual/ja/pdo.installation.php) および対応するデータベース・ドライバ (MySQL データベースのための `pdo_mysql` など) をインストールしなければなりません。
...
[Nginx](http://wiki.nginx.org/) を使うためには、PHP を [FPM SAPI](https://php.net/manual/ja/install.fpm.php) としてインストールしなければなりません。
