...
[PDO](https://php.net/manual/ja/book.pdo.php) の上に構築された Yii DAO (データベース・アクセス・オブジェクト) は、
...
詳細は [PHP マニュアル](https://php.net/manual/ja/function.PDO-construct.php) を参照して下さい。下記にいくつかの例を挙げます。
...
パラメータ・バインディングは [プリペアド・ステートメント](https://php.net/manual/ja/mysqli.quickstart.prepared-statements.php) によって実装されています。
...
> `\Exception` は PHP 7.0 以降では、[`\Throwable` インタフェイス](https://php.net/manual/ja/class.throwable.php) を実装しています。
