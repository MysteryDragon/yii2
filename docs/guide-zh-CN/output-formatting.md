...
> PHP 方法 [date()](https://php.net/manual/en/function.date.php) 就足够了。
...
同时你也可以配置它使用 [PHP date() 语法](https://php.net/manual/en/function.date.php)，只需要加上 `php:` 前缀即可。
例如，
...
[NumberFormatter class](https://php.net/manual/en/class.numberformatter.php) 来进行格式化。
...
> Note: Yii formatter 依赖 [PHP intl extension](https://php.net/manual/en/book.intl.php) 
> 来进行本地数据格式化
