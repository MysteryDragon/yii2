...
Построенные поверх [PDO](https://php.net/manual/ru/book.pdo.php), Yii DAO (объекты доступа к данным) обеспечивают
...
Формат DSN отличается для разных баз данных. Дополнительное описание смотрите в [справочнике PHP](https://php.net/manual/ru/pdo.construct.php).
...
Привязка переменных реализована через [подготавливаемые запросы](https://php.net/manual/ru/mysqli.quickstart.prepared-statements.php).
...
> `\Exception` реализует интерфейс [`\Throwable` interface](https://php.net/manual/ru/class.throwable.php)
